...
3.	Program a function NearestNeighbor in python. This function should take as input the training data (i.e., the DataFrame containing the data from [mixture.csv](./mixture.csv)) and provide the class prediction for a given X1 and X2 coordinate based on the single Nearest Neighbor (1-NN) using Euclidean distance.
...
5.	Challenge: implement a NearestNeighbor function that takes the desired neighborhood size as an input variable. Use it to make predictions based on the 15 Nearest Neighbors (15-NN) for [mixture_test.csv](./mixture_test.csv) and plot the result.
